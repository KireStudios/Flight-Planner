...
class GraphVisualizer:
    ...
    def create_widgets(self):
        ...
        self.cid = self.fig.canvas.mpl_connect('button_press_event', self.on_click)
        # root.bind("<Button>", self.on_click) #FIX
    ...
    def PopupSelect(self, event, selection="default"):
        # Obtenemos las coordenadas absolutas del clic en relación con la ventana raíz
        xlim = self.ax1.get_xlim()
        ylim = self.ax1.get_ylim()

        # Normalize the event data to canvas dimensions
        canvas_width = self.canvas.get_tk_widget().winfo_width()
        canvas_height = self.canvas.get_tk_widget().winfo_height()

        # Map the data coordinates to pixel coordinates
        x = self.canvas.get_tk_widget().winfo_rootx() + (event.xdata - xlim[0]) / (xlim[1] - xlim[0]) * canvas_width
        y = self.canvas.get_tk_widget().winfo_rooty() + (event.ydata - ylim[0]) / (ylim[1] - ylim[0]) * canvas_height
    
        print(f"Popup position: ({x}, {y})")

        # Creamos una nueva ventana emergente
        win = tk.Toplevel()
        win.geometry(f"100x50+{int(x)}+{int(y)}")  # Posicionamos la ventana en la posición del clic
        win.wm_overrideredirect(True)  # Eliminar la barra de título para hacerla más discreta
    
        # Agregar los botones o widgets a la ventana emergente
        neighbors = tk.Button(win, text="Neighbors", command=self.ShowNeighbors)
        neighbors.grid(row=0, column=0)
    
        # Ejemplo de agregar más botones
        # node_add = tk.Button(win, text="Add Node", command=self.AddNode)
        # node_add.grid(row=0, column=1)

        win.mainloop()

    def on_click(self, event):
        if event.button == 3 or True:   #FIX
            click_range = 5 #FIX
            min_distN = min_distS = click_range
            selected_node = None
            for node in self.graph.nodes:
                dist = ((event.xdata - node.coords_x) ** 2 + (event.ydata - node.coords_y) ** 2) ** 0.5
                if dist < min_distN and dist < click_range:
                    min_distN = dist
                    selected_node = node
            selected_segment = None
            for segment in self.graph.segments:
                dist = ((event.xdata - node.coords_x) ** 2 + (event.ydata - node.coords_y) ** 2) ** 0.5
                if dist < min_distS and dist < click_range:
                    min_distS = dist
                    selected_segment = segment
            if min_distN <= min_distS:
                if selected_node:
                    self.PopupSelect(event,"node")
                    return 0
            else:
                if selected_segment:
                    self.PopupSelect(event,"segment")
                    return 0
            self.PopupSelect(event)
